...
//
// Compile:
//   g++ clean_adaptive_IS_simple_sampler.cc -o simple \
...
static BiasParams proposeCandidate(const BiasParams& best, int iter, mt19937& rng) {
    (void)rng; // kept in the function signature so the rest of the code stays unchanged.

    BiasParams p = best;

    // Iteration 1 changes a, iteration 2 changes b, etc.
    const int paramIndex = (iter - 1) % 6;

    // First pass tries positive steps, second pass tries negative steps, then repeat.
    const int pass = (iter - 1) / 6;
    const double direction = (pass % 2 == 0) ? +1.0 : -1.0;

    // Reduce step size slowly after every full positive+negative cycle.
    const int fullCycle = (iter - 1) / 12;
    const double scale = max(0.20, pow(0.75, fullCycle));

    // Base step sizes. The parameters have different natural scales, so the
    // steps are not all the same.
    const double stepA = 0.35 * scale;
    const double stepB = 0.06 * scale;
    const double stepC = 0.08 * scale;
    const double stepE = 0.15 * scale;
    const double stepF = 0.20 * scale;
    const double stepG = 0.20 * scale;

    if (paramIndex == 0) p.a_pT        += direction * stepA;
    if (paramIndex == 1) p.b_xprod     += direction * stepB;
    if (paramIndex == 2) p.c_Q2        += direction * stepC;
    if (paramIndex == 3) p.e_logpT     += direction * stepE;
    if (paramIndex == 4) p.f_satpT     += direction * stepF;
    if (paramIndex == 5) p.g_tworegion += direction * stepG;
