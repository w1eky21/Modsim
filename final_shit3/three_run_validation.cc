...
// Run three comparable Pythia8 samples in one executable:
//   1) naive inclusive HardQCD sample
//   2) Pythia pTHatMin = 50 GeV benchmark sample
//   3) your fixed hybrid LHAPDF bias sample using the best parameters found so far
//
// Output:
//   <prefix>_naive.csv
//   <prefix>_ptcut50.csv
...
//
// Compile example:
//   g++ three_run_validation.cc -o three_run_validation \
...
//
// Run example:
//   ./three_run_validation 100000 validation 12345 NNPDF23_lo_as_0130_qed
//
// Optional arguments:
...
static Summary runSample(const RunConfig& cfg,
    ...
    for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
        ...
        const double gluonLumi = safeGluonLuminosity(pdf.get(), x1, x2, Q);
        const double biasFactor = cfg.useBias ? computeBiasFactor(pars, pTHat, tau, gluonLumi) : 1.0;
        const double pythiaWeight = pythia.info.weight();
        const double manualWeight = cfg.useBias ? pythiaWeight / biasFactor : pythiaWeight;
        const double analysisWeight = cfg.useBias ? manualWeight : pythiaWeight;
...
int main(int argc, char* argv[]) {
    ...
    cout << "Three-run validation with " << nEvents << " requested events per sample\n";
