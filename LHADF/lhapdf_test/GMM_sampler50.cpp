...
//  Constants
// ─────────────────────────────────────────────
constexpr double PT_MIN    =   10.0;   // GeV - phase space floor, well below target
constexpr double PT_MAX    = 7000.0;   // GeV
constexpr double PT_TARGET =   50.0;   // GeV - what we want to sample well above
constexpr double PT_REF    =   50.0;   // GeV - reference scale for boost term
constexpr double MU_F      =   91.2;   // factorisation scale
constexpr int    GLUON     =    21;
constexpr int    N_EVENTS  =  5000;
constexpr int    N_ITER    =   20;

constexpr double NEFF_LOW    = 0.10;
constexpr double NEFF_HIGH   = 0.50;
constexpr double FRAC_TARGET = 0.10;   // want at least 10% of events above 50 GeV
...
// ─────────────────────────────────────────────
struct Diagnostics {
    double Neff, frac_target, mean_w, coverage;
    int    n_above;
};
...
std::vector<Event> run_batch(Grid3D& grid, double boost,
    ...
    pythia.readString("PhaseSpace:pThatMin = 10.");   // floor well below 50 GeV target
...
// ─────────────────────────────────────────────
//  Adapt n from diagnostics
// ─────────────────────────────────────────────
double adapt_boost(double current_boost, const Diagnostics& d) {
    double new_boost = current_boost;

    if (d.frac_target == 0.0 && d.Neff > NEFF_LOW) {
        double step = std::max(0.5, 2.0 / std::sqrt(current_boost));
        new_boost = current_boost + step;
        printf("  [n] No target events yet, increasing: %.2f -> %.2f\n",
               current_boost, new_boost);

    } else if (d.Neff < NEFF_LOW) {
        double step = std::max(0.3, current_boost * 0.15);
        new_boost = current_boost - step;
        printf("  [n] Neff/N=%.3f too low, decreasing: %.2f -> %.2f\n",
               d.Neff, current_boost, new_boost);

    } else if (d.frac_target > 0.0 && d.frac_target < FRAC_TARGET
               && d.Neff > NEFF_LOW) {
        new_boost = current_boost + 0.25;
        printf("  [n] frac=%.4f below target, nudging: %.2f -> %.2f\n",
               d.frac_target, current_boost, new_boost);

    } else if (d.frac_target >= FRAC_TARGET && d.Neff > NEFF_HIGH) {
        new_boost = current_boost - 0.1;
        printf("  [n] Good frac & Neff, refining: %.2f -> %.2f\n",
               current_boost, new_boost);

    } else {
        printf("  [n] n=%.2f stable\n", current_boost);
    }
...
void print_iter(int iter, double boost,
    ...
    printf("\n── Iter %-3d | n=%-5.2f | Neff/N=%-7.4f | "
           "frac>50GeV=%-9.5f | coverage=%.2f\n",
           iter, boost, d.Neff, d.frac_target, d.coverage);

    // histogram centred around the 50 GeV target
    // bins span from PT_MIN up to high pT so full picture is visible
    ...
    for (int b = 0; b < nbins; ++b) {
        ...
        for (const auto& e : events)
            if (e.pT >= edges[b] && e.pT < edges[b+1])
                { ++cnt; sw += e.w; }
        // mark the target bin
        const char* tag = (edges[b] < PT_TARGET && edges[b+1] > PT_TARGET)
                        ? " <-- target" : "";
...
int main() {
    ...
    printf("Phase space floor: %.0f GeV (well below target)\n", PT_MIN);
    printf("Bias: b = q0(pT,x1,x2) * (pT/%.0f)^n * corr3D(pT,x1,x2)\n",
           PT_REF);
    printf("n starts at %.1f and adapts automatically\n", boost);
    printf("%s\n", std::string(65,'-').c_str());
    ...
    for (int iter = 0; iter < N_ITER; ++iter) {
        ...
        if (d.Neff > NEFF_LOW && d.frac_target > best_frac) {
            best_frac  = d.frac_target;
            best_boost = boost;
            best_Neff  = d.Neff;
        }
