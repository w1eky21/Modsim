// five_run_validation.cc
// Run five comparable Pythia8 samples in one executable:
...
//   4) fitted inverse-q0 bias sample, b(pTHat)=(1+pTHat/p0)^n
//   5) your fixed hybrid LHAPDF bias sample using the best parameters found so far
//
// Output:
//   <prefix>_naive.csv
//   <prefix>_ptcut50.csv
//   <prefix>_powerlaw_bias.csv
//   <prefix>_invq0_fit_bias.csv
//   <prefix>_hybrid_bias.csv
//   <prefix>_summary.csv
//
/*
g++ five_run_validation.cc -o five_run_validation \
...
//
// Run example:
//   ./five_run_validation 100000 validation 12345 NNPDF31_lo_as_0118
//
// Optional arguments:
...
//
// Important:
//   For the biased run this file writes both pythiaWeight and manualWeight.
//   analysisWeight defaults to manualWeight for the biased run, because this matches
//   the manual-unbiasing logic used in your previous random-search code. If this
//   double-corrects your weights, use pythiaWeight in the Python plotting script as
//   a cross-check.
...
static double computeInvQ0FitBias(double pTHat) {
    // Fitted from validation_naive.csv using
    // q0(pTHat) = A * (1 + pTHat / p0)^(-n).
    // The inverse proposal bias is proportional to (1 + pTHat / p0)^n.
    const double p0 = 1.520286;
    const double n = 4.956552;
    const double maxBias = 1.0e6;

    double bias = pow(1.0 + pTHat / p0, n);
...
class InvQ0FitBias : public UserHooks {
public:
    bool canBiasSelection() override { return true; }
    ...
    double biasSelectionBy(const SigmaProcess* sigmaProcessPtr,
        ...
        const double pTHat = phaseSpacePtr->pTHat();
        return computeInvQ0FitBias(pTHat);
    }
};
...
static Summary runSample(const RunConfig& cfg,
    ...
    // This changes the proposal distribution approximately by a factor
    //     b(pTHat) = (pTHat / pTRef)^power
    // and Pythia includes the compensating inverse factor in info.weight().
    ...
    // Custom fitted inverse-q0 UserHooks bias.
    if (cfg.useInvQ0FitBias) {
        pythia.setUserHooksPtr(make_shared<InvQ0FitBias>());
    }

    // Custom hybrid LHAPDF/UserHooks bias.
    ...
    for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
        ...
        } else if (cfg.useInvQ0FitBias) {
            biasFactor = computeInvQ0FitBias(pTHat);
        } else if (cfg.usePowerLawBias) {
            // Diagnostic only: Pythia already accounts for this in pythia.info.weight().
            biasFactor = pow(max(pTHat / cfg.powerLawRef, 1.0e-12), cfg.powerLawPow);
        }

        const double pythiaWeight = pythia.info.weight();

        // For the built-in Pythia power-law bias, use Pythia's own compensated weight.
        // For the custom UserHooks biases, manually divide out the selection bias.
        const bool manualUnbias = cfg.useHybridBias || cfg.useInvQ0FitBias;
        const double manualWeight = manualUnbias ? pythiaWeight / biasFactor : pythiaWeight;
        const double analysisWeight = manualUnbias ? manualWeight : pythiaWeight;
...
int main(int argc, char* argv[]) {
    ...
    cout << "Five-run validation with " << nEvents << " requested events per sample\n";
    ...
    vector<RunConfig> configs = {
        // label, pTHatMin, useHybridBias, usePowerLawBias, useInvQ0FitBias, powerLawPow, powerLawRef
        {"naive", 0.0, false, false, false, powerLawPow, 15.0},
        {"ptcut50", 50.0, false, false, false, powerLawPow, 15.0},
        {"powerlaw_bias", 0.0, false, true, false, powerLawPow, 15.0},
        {"invq0_fit_bias", 0.0, false, false, true, powerLawPow, 15.0},
        {"hybrid_bias", 0.0, true, false, false, powerLawPow, 15.0}
    };
