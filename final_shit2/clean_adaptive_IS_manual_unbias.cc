// Simple adaptive multivariable phase-space bias search for rare high-pT QCD events in Pythia8
//
// What this code does:
...
//
// Compile:
//   g++ clean_adaptive_IS_manual_unbias.cc -o clean \
...
static double balancedScore(double fracPT50, double neffRatio) {
    const double denom = fracPT50 + neffRatio;
    if (denom <= 0.0) return 0.0;
    return 2.0 * fracPT50 * neffRatio / denom;
}
...
static BatchResult runBatch(const BiasParams& pars,
                            ...
                            bool saveEvents) {
    Pythia pythia;
    configurePythia(pythia);
    pythia.setUserHooksPtr(make_shared<MultiVariableBias>(pars));
...
// ------------------------------------------------------------
// Explanation:
// Creates the next candidate by perturbing the best candidate so far. Every
// eighth step it tries a broader random point. This keeps the code simple while
// still searching all bias parameters.
// ------------------------------------------------------------
